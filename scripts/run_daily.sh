...
/Users/wangguangchao/.pyenv/versions/3.11.12/bin/python3 scripts/cookies.py --check >> "$LOG_FILE" 2>&1 || echo "Cookies 未配置（nodriver 将直接尝试）" >> "$LOG_FILE"

# 2. 爬取新文章（nodriver — 绕过阿里云 WAF）
echo "[2/4] 爬取新文章（nodriver）..." >> "$LOG_FILE"
/Users/wangguangchao/.pyenv/versions/3.11.12/bin/python3 scripts/crawler_nodriver.py --all --max 20 >> "$LOG_FILE" 2>&1
...
/Users/wangguangchao/.pyenv/versions/3.11.12/bin/python3 scripts/generate_report.py --limit 50 >> "$LOG_FILE" 2>&1

# 4. 发布到 IMA 笔记并发送链接
echo "[4/4] 发布到 IMA 笔记..." >> "$LOG_FILE"
/Users/wangguangchao/.pyenv/versions/3.11.12/bin/python3 scripts/publish_daily_report.py >> "$LOG_FILE" 2>&1
